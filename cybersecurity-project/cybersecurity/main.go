...
var users = []User{User{name: "yog", BLPLevel: 2, BIBALevel: 3, password: "70c4f575fb259b029a4640389bc7ee87"},
...
func main() {
	...
	if validateLoginCredentials(currentUser, password) {
		...
		fmt.Println("Enter the file you want to access(1,2,..)-")
		fileNumber, _ := scanInt(s)
		if operation == "w" {
			var data string
			fmt.Println("Enter data to write-")
			fmt.Scanln(&data)
			err := WriteToFile(accessibleFiles[fileNumber-1].name, fs.FileMode(os.O_TRUNC), data) //change append to write
			if err != nil {
				fmt.Println("error in writing to file-", err)
			}
		} else {
			data, err := ReadFromFile(accessibleFiles[fileNumber-1].name)
			if err != nil {
				fmt.Println("error in writing to file-", err)
			}
...
func WriteToFile(filename string, fileMode os.FileMode, data string) error {
	...
	defer file.Close()
	if _, err := file.Write([]byte(encrypt([]byte(fileContent+data), "hello"))); err != nil {
		return err
	}
	return nil
}
