...
func encrypt(data []byte, passphrase string) string {
	...
	fmt.Println("cmacHash-", string(hash))
...
func checkIfMsgIsAuthenticated(msgReceived string, cmacGiven string, passphrase string) bool {
	...
	fmt.Println("hashObtained in checking-", hashObtained)
...
func main() {
	...
	if checkIfMsgIsAuthenticated(cipherText[32:], cipherText[:32], "hello") {
		decryptedData := decrypt([]byte(cipherText), "hello")
		fmt.Println(string(decryptedData))
	} else {
		fmt.Println("msg is modified in between")
	}
