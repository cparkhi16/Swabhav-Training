...
func decrypt(myarr []float64) {
	var decr []rune
	for _, i := range myarr {
		fmt.Println(math.Round(math.Mod((i-float64(key[1]))*float64(findInverse(key[0])), 26)) + 97)
		decr = append(decr, rune(math.Round(math.Mod((i-float64(key[1]))*float64(findInverse(key[0])), 26))+97))
	}
	for _, i := range decr {
