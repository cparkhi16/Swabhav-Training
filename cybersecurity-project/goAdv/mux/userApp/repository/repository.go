...
type Repository interface {
	...
	GetCount(uow *UnitOfWork, entity interface{}, count interface{}, queryProcessors []QueryProcessor) error
}
...
func Not(condition string, args ...interface{}) QueryProcessor {
	return func(db *gorm.DB, out interface{}) (*gorm.DB, error) {
		db = db.Not(condition, args...)
		return db, nil
	}
}
...
func (repository *GormRepository) GetCount(uow *UnitOfWork, entity interface{}, count interface{}, queryProcessors []QueryProcessor) error {
	db := uow.DB
	if queryProcessors != nil {
		var err error
		for _, queryProcessor := range queryProcessors {
...
func (repository *GormRepository) Delete(uow *UnitOfWork, entity interface{}) error {
	//uow.DB.Debug().Association("Courses").Delete(entity)
