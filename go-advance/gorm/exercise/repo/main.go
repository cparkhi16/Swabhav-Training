...
func main() {
	...
	employeeService.GetEmpNameDeptNameWithNoEmployeeAsNull(&dept, "ename as ENAME,dept.dname as DNAME,job as JOB", "left join emp on emp.deptno=dept.deptno")
	employeeService.GetDeptNameWhereEmployeesAreNull(&dept, "dept.dname as DeptName", "left join emp on emp.deptno=dept.deptno")
	...
	employeeService.GetEmpBossAndDeptName(&ep, "e.ename as ENAME ,m.ename as ENAME ,d.dname as DNAME", "e left join emp m on e.mgr=m.empno", " join dept d on e.deptno=d.deptno")
	...
	regionService.GetCountriesWithNoStates(&c, "`countries`.*", " join locations on countries.COUNTRY_ID = locations.COUNTRY_ID", "countries.country_id")
