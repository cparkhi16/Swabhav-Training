...
app.post('/api/v1/register', async (req, resp) => {
  ...
  let userHashedPassword = hashIt(password)
  console.log("Hashed pwd ", userHashedPassword)
  req.body.password = userHashedPassword;
  ...
  if (req.body.constructor === Object && Object.keys(req.body).length === 0) {
    ...
    user.create(new_user, function (err, user) {
      ...
      resp.json({ error: false, message: "user added successfully!", data: user });
    });
  }
});
