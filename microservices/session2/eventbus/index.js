...
app.post('/eventbus/event',(req,resp)=>{// when a event occurs store it in events list (used by all svcs)
    ...
    // events.push(event)
    axios.post("http://localhost:4001/eventbus/event/listener",event).catch(e=>console.log(e.message))//blogpost
    axios.post("http://localhost:4002/eventbus/event/listener",event).catch(e=>console.log(e.message))//blogcomment
    axios.post("http://localhost:4003/eventbus/event/listener",event).catch(e=>console.log(e.message))//query
    // resp.send({})
    console.log("Here ")
