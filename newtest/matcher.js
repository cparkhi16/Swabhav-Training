...
function addFilterByKeyword(filter, keyword, map) {
 ...
 // console.log(" within add filters by keyword",filter,keyword,map)
  let list = map.get(keyword);
...
function matchFilter(filter, request, typeMask, sitekey, collection) {
  //console.log("filtr match ---------------",filter)
  // filter.name = "Chinmay"
...
exports.Matcher = class Matcher {
  ...
  add(filter) {
    ...
    //console.log(" simple >>>",simple,filter.contentType,filter.isGeneric())
    ...
    if (simple) {
        //console.log("compiled patterns keyword  ",this._compiledPatternsByKeyword)
      if (this._compiledPatternsByKeyword.size > 0)
        this._compiledPatternsByKeyword.delete(keyword);
        //console.log(" Returning ",this._simpleFiltersByKeyword)
      return;
    }
    ...
    // this._filterDomainMapsByKeyword.clear();
    // this._filterMapsByType.clear();
    // console.log(this._filterDomainMapsByKeyword)
    // console.log(this._filterMapsByType)
    // console.log(this._keywordByFilter)
    // console.log(this._simpleFiltersByKeyword)
    // console.log(this._compiledPatternsByKeyword)
    // console.log(this._complexFiltersByKeyword)
  }
   ...
   */
  findKeyword(filter) {
    //console.log(" in findketword ------------",filter)
    let result = "";

    let candidates = filter.urlPattern.keywordCandidates();
    if (!candidates)
    {
      //console.log(" Candidates from keyword candidates ",candidates)
      return result;
    }
   // console.log(" Candidates from keyword candidates ==",candidates)
    ...
    //console.log(" Last return ---------- ",result)
    return result;
  }
  ...
  _checkEntryMatchSimpleQuickCheck(keyword, request, filters) {
   ...
   // console.log(" compiled in simple check ",compiled)
    if (typeof compiled == "undefined") {
      compiled = compilePatterns(filters);
     // console.log(" got a compiled pattern ",compiled,keyword)
      this._compiledPatternsByKeyword.set(keyword, compiled);
     // console.log(" got a compiled pattern ",compiled,keyword,this._compiledPatternsByKeyword)
    }

    // If compilation failed (e.g. too many filters), return true because this
    // is only a pre-check.
    //console.log("-== compiled and compiled test ==- ",compiled, !compiled)
    return !compiled || compiled.test(request);
  }

  _checkEntryMatchSimple(keyword, request, collection) {
    // console.log("_checkEntryMatchSimple",keyword)
     //console.log("simplekeysfilter map",this._simpleFiltersByKeyword)
    let filters = getFilters(this._simpleFiltersByKeyword, keyword);
    //console.log(" all filters --------------------- >>> ",filters)
    let {length} = filters;
    //console.log("Length in _checkEntryMatchSimple",length)
    if (length) {
     ...
     // console.log(" quick check ---= ",this._checkEntryMatchSimpleQuickCheck(keyword, request, filters))
      if (length === 1 ||
  ...
  _checkEntryMatchForType(keyword, request, typeMask, sitekey, specificOnly,
    ...
    //console.log("_checkEntryMatchForType")
    if (filtersForType) {
        //console.log("filterdForType",filtersForType)
  ...
  _checkEntryMatchByDomain(keyword, request, typeMask, sitekey, specificOnly,
                           collection) {
    let filtersByDomain = this._filterDomainMapsByKeyword.get(keyword);
    //console.log("_checkEntryMatchByDomain",keyword,filtersByDomain)
    if (filtersByDomain) {
        //console.log(" filtersbydomain =",filtersByDomain)
      if (filtersByDomain instanceof FiltersByDomain) {
  ...
  checkEntryMatch(keyword, request, typeMask, sitekey, specificOnly,
    ...
    //console.log(" RESOURCE Type ",RESOURCE_TYPES )
    // console.log("keyword ",keyword)
    // console.log("request ",request)
    //console.log("typeMask ",typeMask)
    // console.log("siteKey ",sitekey)
    // console.log("specific only ",specificOnly)
    // console.log("collection ",collection)
    //console.log("------------------------------------------------",(typeMask & RESOURCE_TYPES))
    if (!specificOnly  && (typeMask & RESOURCE_TYPES) != 0 ) {
        //console.log("here in 1st if", this._simpleFiltersByKeyword.size)
 ...
 match(url, typeMask, docDomain, sitekey, specificOnly) {
   ...
   // console.log("-= Request -= ",request)
    ...
    for (let i = 0, l = candidates.length; i < l; i++) {
      // if( candidates[i]=="wxuh" || candidates[i]=="6dtxqbu2vlgf4rxoezvmxoxs90aaqwxealw"){
      //   console.log(" got -------------------------")
      // }
...
exports.Matcher.hello = function(url, typeMask, docDomain, sitekey, specificOnly){
    //console.log(" hello world ",this._simpleFiltersByKeyword)
   // let a = new Matcher()
    // let b = new Filter("||example.com/ad.png")
    // a.add(b)
    // console.log(b)
  //let fE =  filterEngine
  //fE.initialize()
    // let text = Filter.normalize("*.png");
    //   if (!text)
    //     throw "Attempted to create a filter from empty text";
    // let newFilter = Filter.fromText(text);

    // let t = Filter.normalize("&ad_ids=");
    //   if (!t)
    //     throw "Attempted to create a filter from empty text";
    // let newFilter2 = Filter.fromText(t);
    
    // //console.log("new filter ",newFilter2,newFilter)
    // a.add(newFilter)
    // a.add(newFilter2)
    let x= defaultMatcher.match(url, typeMask, docDomain, sitekey, specificOnly)
    //console.log(" Result from match >>> ",x)
   // this.match(url, typeMask, docDomain, sitekey, specificOnly)
};
let CombinedMatcher =
/**
...
exports.CombinedMatcher = class CombinedMatcher {
   ...
   */
  add(filter) {
    //console.log(" in combined matcher add")
