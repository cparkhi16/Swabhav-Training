...
//const {visibleRecommendations} = require("./recommendations");
...
function preload(subscription,info)
{
...
let registerSubscription = exports.registerSubscription =   function(subscription,info)
{
  filterStorage.addSubscription(subscription);
   preload(subscription,info);
  // if (!subscription.lastDownload && Prefs.synchronization_enabled)
...
async function startEngine()
{
  ...
  {
    let node = Utils.chooseFilterSubscription([...visibleRecommendations()]);
    if (node)
    {
    ...
    if ([...filterStorage.subscriptions()].find(it => it.url === url) === undefined)
    {
      ...
      aaSubscription.disabled = !Prefs.acceptable_ads_enabled;
