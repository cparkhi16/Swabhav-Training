...
const { URLFilter, ContentFilter } = require("./filterClasses");
...
class ElemHide {
  ...
  add(filter) {
    ...
    if (!(domains || elemHideExceptions.hasExceptions(selector))) {
      ...
      console.log(" ADDING filter ----------== ",filter, filter instanceof ContentFilter)
      ...
      console.log(" adding in filters by domain --= ",filter)
  ...
  getStyleSheet(domain, specificOnly = false, includeSelectors = false,
    ...
    if (specificOnly) {
      ...
      console.log(" known suffix -== ",knownSuffix)
      if (includeSelectors || includeExceptions) {
        ...
        console.log(" within else ",this._filterBySelector.size)
        code = knownSuffix == "" ? this._getCommonStyleSheet() :
