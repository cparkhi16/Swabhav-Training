...
exports.compilePatterns = function compilePatterns(filters) {
  ...
  for (let filter of filters) {
    let source = filter.urlPattern.pattern;
    //console.log("----- ",filter.urlPattern.pattern)
    if (filter.matchCase)
      caseSensitive += source + "|";
    else
      caseInsensitive += source + "|";
  }
  // console.log(" cases ",caseInsensitive)
  ...
  try {
    ...
    //console.log(" error creating regex for this ---< string ---",caseInsensitive)
    return null;
  }
  //console.log(" case insensitive regexp ",caseInsensitiveRegExp)
...
exports.Pattern = class Pattern {
  ...
  constructor(pattern, matchCase) {
    ...
    if (pattern.length >= 2 &&
      ...
      //console.log(" IN first else of constr ",pattern)
      this._regexp = new RegExp(pattern);
    }
    else {
      //console.log("here in else ==============",pattern)
   ...
   // ^[\w\-]+:\/+(?:[^\/]+\.)?$  (?:[\x00-\x24\x26-\x2C\x2F\x3A-\x40\x5B-\x5E\x60\x7B-\x7F]|$)\[\\w\\\-\]\+\:\\\/\+\(\?\:\[(?:[\x00-\x24\x26-\x2C\x2F\x3A-\x40\x5B-\x5E\x60\x7B-\x7F]|$)\\\/\]\+\\\.\)\?\$
    let te = filterToRegExp(filterToRegExp("||") + "$")
   // console.log("------++ Patterns _++++ ",this.pattern,te,new RegExp(te))
  }
  ...
  matchesLocation(request) {
   // console.log(" in matches location ------ ",request)
    ...
    while (index != -1) {
      ...
      if ((startsWithExtendedAnchor ?
          ...
          (endsWithSeparator ?
           ...
           true))
           {
            //console.log(" returning true in matches location ",index,extendedAnchorRegExp.test(location.substring(0, index)),location.substring(0, index),location[index + pattern.length])
            return true;
           }
      ...
      //console.log("within while ",index)
    }
  ...
  keywordCandidates() {
    if (!this.pattern)
    {
      //console.log(" returning bc of not pattern")
      return null;
    }
