...
exports.Subscription = class Subscription {
  ...
  updateFilterText(filterText) {
    ...
    //console.log(" -------------------------------- adding _filterText completed")
    //throw "exit"
...
exports.Subscription.fromURL = function(url) {
  ...
  //console.log(" known subs --===<< ",subscription)
  if (subscription)
    return subscription;
...
exports.Subscription.fromObject = function(obj) {
  ...
  if (obj.url[0] != "~") {
    ...
    if ("metadata" in obj)
      result._metadata = JSON.parse(obj);
  }
...
class DownloadableSubscription extends RegularSubscription {
   ...
   */
  constructor(url, title) {
    //console.log(" Within downlaodable subscription == ",url,title)
    super(url, title);
