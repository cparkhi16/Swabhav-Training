...
function selectModule(type) {
 // console.log(" select module ",type)
  switch (type) {
    case "blocking":
    case "allowing":
      //console.log(" returning defaultMatcher",defaultMatcher)
...
//    this.p = profiler("FilterEngine");
  ...
  initialize(filterSource) {
    //console.log("+++++++++++++++++ FILTER ENGINE INIT +++++++++++++++++++++++++++++++")
   // this.p.mark("initialize");
    if (!this._initializationPromise) {
      if (typeof filterSource == "undefined") {
        ...
        //console.log("++++++++ in undefined filtersource ++++++++++++++")
   ...
   */
  add(filter) {
    //console.log("my filter ",filter.type)
    let module = selectModule(filter.type); // needs to be blocking 
    console.log(" module ======= ",module)
    if (module)
      module.add(filter);
    // console.log("default matcher ",defaultMatcher)
    // defaultMatcher.add(filter)
    // let res = module.match("http://example.com/ad.png",4,"example.com","",false)
    // console.log(" result from match ",res)
  }

  // RBI CHECK 
  checkUrl(url, typeMask, docDomain, sitekey, specificOnly){
    let res = defaultMatcher.match(url, typeMask, docDomain, sitekey, specificOnly)
    console.log(" result from match ",res)
  }
  ...
  match(url, typeMask, documentHostname, sitekey = null, specificOnly = false) {
    // /console.log(" __________________ MATCH OF FILTER ENGINE ___________________________")
