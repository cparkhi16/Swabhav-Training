...
function parseFilterList(filters) {
  //console.log("in parse list",filters)
  ...
  for (let {hasOwnProperty} = Object.prototype, i = 1; i < lines.length; i++) {
    ...
    //console.log("________________________________ process filter list match ",match,lines)
    if (!match)
      break;

    let keyword = match[1].toLowerCase();
    if (hasOwnProperty.call(params, keyword)) {
      //console.log(" in if of has own property",keyword)
