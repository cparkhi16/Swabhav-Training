...
exports.parse = function parse(text) {
  ...
  //console.log(text,CONTENT_FILTER.test(text))
  ...
  if (text.startsWith("@@")) {
   ...
   // console.log(" orig text and text ",text,origText)
  }
  ...
  if (match) {
    //console.log("+++++++++++++++ Within parse match ",text)
  ...
  //console.log("-------------- my parse data ",blocking,origText,text,matchCase,contentType)
  return {
    blocking,
    text: origText,
    regexpSource: text,
...
exports.normalize = function(text, dontSanitize = false) {
  //console.log("text in normalize ",normalize)
  // if(text === "&action=getads&"){

  //   console.log("=============== BREAKING =========================");
  //   throw  new Error(" hey ")
  // }
  //console.log(" dont sanitize",text)
  if (text === "")
    return text;
  ...
  if (!text.includes(" "))
  {
    //console.log(" returing as text doesn't include empty space ",text);
    return text;
  }
  ...
  if (/^ *!/.test(text))
  {
    //console.log(" within don't remove spaces inside comments ")
    return text.trim();
  }
