...
exports.Filter.fromText = function(text) {
  ...
  if (text.length === 0) {
    filter = new InvalidFilter(text, "filter_empty"); // log invalid filter
  }
  else if (text[0] == "!") {
    filter = new CommentFilter(text);  // log invalid filter
  }
  else {
    let match = text.includes("#") ? CONTENT_FILTER.exec(text) : null;
    console.log(" matches with # -===- ",match)
    if (match)
      filter = ContentFilter.fromText(text, match[1], match[2], match[3]);// log invalid filter
    else
      filter = URLFilter.fromText(text);
      //console.log(" new filter val ========== ",filter)
  }
...
exports.ActiveFilter = class ActiveFilter extends Filter {
  ...
  *serialize() {
  ...
  isGeneric() {
    ...
    //console.log(" sitekeys and domains ",sitekeys,domains)
...
exports.URLFilter = class URLFilter extends ActiveFilter {
  ...
  constructor(text, regexpSource, contentType, matchCase, domains, thirdParty,
     ...
     * @type {number}
     */
    this.contentType = contentType != null ? contentType : RESOURCE_TYPES; //2147483647
  ...
  matches(request, typeMask, sitekey) {
    //console.log("------------------============ Filter MATCHES =====================---------------------",this.urlPattern.matchesLocation(request) )
...
exports.URLFilter.fromText = function(filterText) {
  ...
  try {
    ...
    //console.log(" returing allowing filter ")
...
exports.ContentFilter.fromText = function(text, domains, type, body) {
  ...
  console.log(" -=--=== Domains --==---- ",domains,text)
